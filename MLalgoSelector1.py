...
def recommend_algorithm(problem_type, ml_task, dataset_size, special_case):
    ...
    if problem_type == "Supervised Learning":
        if ml_task == "Classification":
            if "Small" in dataset_size:
                ...
                recommendation += "- Neural Networks, Custom CNNs, LightGBM, XGBoost\n"
    ...
    if special_case == "Imbalanced Data":
        ...
        recommendation += "\n🔹 First, perform **Feature Extraction** using:\n"
        recommendation += "   - Pretrained CNNs (ResNet, EfficientNet, VGG, MobileNet)\n"
        recommendation += "   - Autoencoders for unsupervised feature learning\n"
        recommendation += "   - PCA for dimensionality reduction\n"
        recommendation += "\n🔹 Then, apply ML models on extracted features:\n"
        recommendation += "   - k-NN, SVM, Random Forest, XGBoost\n"
        recommendation += "\n🔹 Alternatively, train a **Custom CNN** end-to-end for deep learning.\n"
